...
Promise.resolve(/* import() eager */).then(__webpack_require__.bind(__webpack_require__, 6451))
...
function LeftSide() {
    ...
    (0,react_.useEffect)(()=>{
        ...
        async function getUsers() {
            try {
                const resp = await axiosInstance/* default */.Z.get("http://localhost:3000/api/user/all");
                if (resp.data === null) {
                    return;
                }
                setUsers(resp.data);
        ...
        async function getProfiles() {
            try {
                const resp = await axiosInstance/* default */.Z.get("http://localhost:3000/api/user/profiles");
                if (resp.data === null) {
                    return;
                }
        ...
        async function getMessages() {
            try {
                const resp = await axiosInstance/* default */.Z.get(`http://localhost:3000/api/chat/messages/${user?.id}`);
                if (resp.data === null) {
                    return;
                }
                resp.data.sort((a, b)=>{
        ...
        async function getMyChannels() {
            ...
            try {
                const resp = await axiosInstance/* default */.Z.get(`http://localhost:3000/api/chat/myChannels/${user?.id}`);
                if (resp.data === null) {
                    return;
                }
        ...
        async function getChannels() {
            console.log("getChannels user : ", user);
            try {
                const resp = await axiosInstance/* default */.Z.get(`http://localhost:3000/api/chat/channels`);
                if (resp.data === null) {
                    return;
                }
