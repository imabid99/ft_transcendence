...
export default function Search({ setShowBody }: Props) {
  ...
  const handelClickProtected = (id: string) => {
    socket?.emit("joinProtectedGroup", {
      groupId: id,
      userId: user?.id,
      password: password,
    });
  ...
  return (
    <div className="flex flex-col  overflow-y-scroll  max-h-[calc(100%-270px)] min-h-[calc(100%-135px)] no-scrollbar relative ">
      <div
        className=" flex gap-2 items-center  py-[27px] px-[25px] cursor-pointer"
            ...
            return (
              ...
              >
                ...
                <div>
                  ...
                  <p>
                    {ur.status === "online" ? (
                      <span className="text-[#6BB279]">Online</span>
                    ) : ur.status === "ofline" ? (
                      <span className="text-[#898F94]">Offline</span>
                    ) : (
                      <span className="text-[#034B8A]">{ur.status}</span>
                    )}
                  </p>
                </div>
              </div>
              ...
              >
                <div className="flex items-center gap-[10px] ">
                  ...
                  <div>
                    ...
                    <span className="text-[#898F94]">{channel.type}</span>
                  </div>
                </div>
                <div
                  className="flex items-center justify-center px-[30px] py-[10px] bg-[#366996] rounded-[10px] text-[#fff] text-[14px] font-[500] font-[Poppins] cursor-pointer self-end"
                  onClick={() =>
                    channel.type === "Protected"
                      ? setIsProtected(channel.id)
                      : handelClickChannel(channel.id, channel.type)
                  }
                >
                  Join
                </div>
      ...
      {isProtected && (
        <>
          <div className="fixed bottom-0 left-0 w-full flex justify-center items-center h-full  z-[100]">
            <div
              className="absolute bottom-0 w-full flex justify-center items-center h-full bg-black bg-opacity-5 backdrop-blur-[1.5px] "
            ...
            <form className="w-[400px]  bg-[#ffffff] flex p-[20px]  flex-col items-center rounded-[30px] z-[1000] gap-[45px] pb-[40px] pt-[30px] shadow-joinGroupModal">
              ...
              <input
                ...
                className="w-[330px] pl-[30px] py-[20px] border-[0.5px] border-[#034B8A] rounded-[20px] text-[16px] font-[400] font-[Poppins] text-[#C6D4E1] focus:outline-none"
                onChange={(e) => setPassword(e.target.value)}
              />
              <span className="flex gap-[10px]">
                <button
                  className="w-[165px] py-[10px] rounded-[15px] text-[16px] font-[500] font-[Poppins] cursor-pointer border-[0.5px] border-[#034B8A] text-[#034B8A]"
                  onClick={() => setIsProtected(null)}
                >
                  Cancel
                </button>
                <button
                  className="w-[165px] py-[10px] bg-[#366996] rounded-[15px] text-[#fff] text-[16px] font-[500] font-[Poppins] cursor-pointer"
                  onClick={() => handelClickProtected(isProtected)}
                >
                  Join
                </button>
              </span>
