...
export class GameGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  async handleConnection(client: Socket) {
    ...
    if (token) {
      ...
      this.waitingPlayers.push(newObjact);
  ...
  async handlePaddleMove(client: Socket, payload: { direction: string, moving: boolean, playerId?: string }) {
  ...
  async handleBallPosition(client: Socket, payload: {x: number, y: number, z: number}) {
    const match = this.matches.get(client.id);
    // if (match) {
      const {matchId, players} = match;
